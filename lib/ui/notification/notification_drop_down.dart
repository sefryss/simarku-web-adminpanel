...
class NotificationDropDown extends StatelessWidget {
  ...
  NotificationDropDown(this.homeController,{
    Key? key,
    this.onChanged,
    this.value,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if(homeController.storyListNotification.length > 0){
      if(value == null){
        onChanged!(homeController.storyListNotification[0].id);
        homeController.storyNotification(homeController.storyListNotification[0].id);
      }else{
        onChanged!(value);
        homeController.storyNotification(value);
      }
    }
    return  homeController.storyListNotification.length > 0
  ...
  getDropDown(
    ...
    return Obx(() => Container(
      ...
      child: DropdownButton<String>(
        ...
        items: homeController.storyListNotification.map((val) {
        ...
        value: (homeController.storyNotification.value.isEmpty)?null:homeController.storyNotification.value,
        underline: Container(),
        onChanged: (value) {
          onChanged!(value);
          homeController.storyNotification(value);
